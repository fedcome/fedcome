...
class client(client):
    ...
    def get_full_grad(self):
        ...
        init = copy.deepcopy(self.get_model_params_vector().detach())
        K = 0
        ...
        with torch.no_grad():
            optimizer.step()
            nxt = torch.nn.utils.parameters_to_vector(model.parameters())
            grad = (init - nxt)/K
        optimizer.zero_grad()
        return grad

    def train(self, server_parameters, state_param_diff, round_idx):
        ...
        for i in range(epoch):
            for (x, y) in self.train_dataloader:
                ...
                scafford_loss = torch.sum(local_param_list*state_param_diff)
                # scafford_loss = 0
                # cur = torch.nn.utils.parameters_to_vector(model.parameters())
                loss = loss + scafford_loss
                loss.backward()
